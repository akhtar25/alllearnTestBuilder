...
if not application.debug and not application.testing:
    ...
    application.logger.info('allLearn startup')
...
@application.route('/gTokenSignin',methods=["GET","POST"])
def gTokenSignin():
    try:
        idtoken = request.form.get('idtoken')
        # Specify the CLIENT_ID of the application that accesses the backend:
        idinfo = id_token.verify_oauth2_token(idtoken, requests.Request(), application.config['GOOGLE_CLIENT_ID'])
        ...
        # "azp": "1008719970978-hb24n2dstb40o45d4feuo2ukqmcc6381.applications.googleusercontent.com",
        # "aud": "1008719970978-hb24n2dstb40o45d4feuo2ukqmcc6381.applications.googleusercontent.com",
        # "iat": "1433978353",
        # "exp": "1433981953",
        #
        # // These seven fields are only included when the user has granted the "profile" and
        # // "email" OAuth scopes to the applicationlication.
...
#@application.route("/api/leaderBoard/<int:schoolID>",methods=['GET','PUT','DELETE'])
...
@application.route("/api/studentList/<int:schoolID>/", methods=['GET', 'PUT', 'DELETE'])
...
@application.route("/api/<int:key>/", methods=['GET', 'PUT', 'DELETE'])
...
@application.route('/practiceTest',methods=["GET","POST"])
def practiceTest():    
    if request.method=="POST":
    ...
    if current_user.is_anonymous:            
        studentProfile = StudentProfile.query.filter_by(user_id=application.config['ANONYMOUS_USERID']).first()  #staging anonymous username f        
    else:
...
@application.route('/embeddable')
@talisman(frame_options=ALLOW_FROM, frame_options_allow_from='*')
def embeddable():
    return 'Embeddable'

@application.route("/loaderio-ad2552628971ece0389988c13933a170/")
...
@application.route('/schoolRegistration', methods=['GET','POST'])
@login_required
def schoolRegistration():
    ...
    if form.validate_on_submit():
        ...
        for det in moduleDetRow:
            ...
            eachList.applicationend(det.module_name)
            eachList.applicationend(det.module_url)
            eachList.applicationend(det.module_type)
            # detList.applicationend(str(det.module_name)+":"+str(det.module_url)+":"+str(det.module_type))
            detList.applicationend(eachList)
...
@application.route('/promoteStudent',methods=['POST','GET'])
@login_required
def promoteStudent():
...
@application.route('/classRegistration', methods=['GET','POST'])
@login_required
...
@application.route('/teacherDirectory',methods=['GET','POST'])
@login_required
def teacherDirectory():
    ...
    if school_name_val ==None:
        ...
        for i in ClassSection.query.with_entities(ClassSection.class_val).distinct().filter_by(school_id=teacher_id.school_id).all():
            class_list.applicationend((str(i.class_val), "Class "+str(i.class_val)))
        for i in available_section:
            section_list.applicationend((i.section,i.section))
...
@application.route('/feeMonthData')
def feeMonthData():
...
@application.route('/feeStatusDetail')
def feeStatusDetail():
    qmonth = request.args.get('month')
    qyear = request.args.get('year')
    class_val = request.args.get('class_val')
...
@application.route('/payrollMonthData')
...
@application.route('/updateFeeData', methods=['GET','POST'])
def updateFeeData():
...
@application.route('/updatePayrollData', methods=['GET','POST'])
...
@application.route('/bulkStudReg')
def bulkStudReg():
    return render_template('_bulkStudReg.html')


@application.route('/singleStudReg')
def singleStudReg():
...
@application.route('/studentRegistration', methods=['GET','POST'])
@login_required
...
@application.route('/video_feed_stop')
def video_feed_stop(): 
    #cam.is_record=False
    cam=VideoCamera()
...
@application.route('/ScanBooks',methods=['GET', 'POST'])
def ScanBooks():
    print ("We're here!")
    return render_template('ScanBook.html',title='Scan Page')


@application.route('/testingOtherVideo',methods=['GET', 'POST'])
...
@application.route('/performanceChart',methods=['GET','POST'])
def performanceChart():
    ...
    if resultSet:
        for result in resultSet:
            ...
            resultArray.applicationend(Array)
        return {'result' : resultArray} 
    else:
        return jsonify(["NA"])

@application.route('/performanceBarChart',methods=['GET','POST'])
def performanceBarChart():
    ...
    for sub in subject_id:
        ...
        performance_array.applicationend(Array)
    return {'performance':performance_array}
    
    

@application.route('/disconnectedAccount')
@login_required
...
@application.route('/postJob',methods=['POST','GET'])
@login_required
def postJob():
    teacherRow=TeacherProfile.query.filter_by(user_id=current_user.id).first()
...
def openJobs():
    ...
    if first_login=='1':
        ...
        db.session.commit()
        flash('Please complete your profile before applicationlying for jobs')
...
@application.route('/openJobsFilteredList')
...
def jobDetail():
    ...
    if current_user.is_anonymous:
        ...
        jobApplicationlicationRow = ''
    else:
        print('user exist')
        jobApplicationlicationRow = JobApplicationlication.query.filter_by(job_id=job_id, applicationlier_user_id=current_user.id).first()
    if jobApplicationlicationRow:
        applicationlied=1
    else:
        applicationlied=0
    if current_user.is_anonymous:
        return render_template('jobDetail.html', title='Job Detail', 
            schoolProfileRow=schoolProfileRow,addressRow=addressRow,jobDetailRow=jobDetailRow,applicationlied=applicationlied)
    else:
        return render_template('jobDetail.html', title='Job Detail', 
            schoolProfileRow=schoolProfileRow,addressRow=addressRow,jobDetailRow=jobDetailRow,applicationlied=applicationlied,user_type_val=str(current_user.user_type))
    



@application.route('/sendJobApplicationlication',methods=['POST','GET'])
@login_required
def sendJobApplicationlication():
    ...
    if request.method=='POST':
        ...
        #teacherRow=TeacherProfile.query.filter_by(user_id=current_user.id).first()
        jobApplicationlyData=JobApplicationlication(applicationlier_user_id=current_user.id, job_id=job_id_form,
                applicationlied_on =datetime.today(),status='Applicationlied',school_id=school_id,available_from=available_from,available_till=available_till,
                last_modified_date=date.today())
        db.session.add(jobApplicationlyData)
        db.session.commit()
        flash('Job applicationlication submitted!')
        #try:            
        ...
        new_applicationlicant_for_job(teacherRow.email,teacherRow.teacher_name,current_user.first_name + ' '+current_user.last_name,jobDetailRow.category)
        #except:
        #    pass
        return redirect(url_for('openJobs'))

@application.route('/applicationliedJobs')  # this page shows all the job posts that the user has applicationlied to
@login_required
def applicationliedJobs():
    applicationliedQuery = "select applicationlied_on, t3.school_id,school_name, category, subject, t2.job_id, "
    applicationliedQuery = applicationliedQuery + "t1.status as applicationlication_status, t2.status as job_status "
    applicationliedQuery = applicationliedQuery + "from job_applicationlication t1 inner join job_detail t2 on "
    applicationliedQuery = applicationliedQuery + "t1.job_id=t2.job_id inner join school_profile t3 on "
    applicationliedQuery = applicationliedQuery + "t3.school_id=t1.school_id where t1.applicationlier_user_id='"+str(current_user.id)+"'"
    applicationliedRows = db.session.execute(text(applicationliedQuery)).fetchall()
    return render_template('applicationliedJobs.html',title='Applicationlied jobs', user_type_val=str(current_user.user_type),applicationliedRows=applicationliedRows)


@application.route('/jobApplicationlications')  # this page shows all the applicationlications received by the job poster for any specifc job post
@login_required
def jobApplicationlications():
    teacher=TeacherProfile.query.filter_by(user_id=current_user.id).first()
    job_id=request.args.get('job_id')
    #jobApplicationlications = JobApplicationlication.query.filter_by(school_id=teacher.school_id).order_by(JobApplicationlication.applicationlied_on.desc()).all()
    #pending descision
    jobApplicationQuery = "select t1.applicationlied_on, t2.first_name, t2.last_name, t2.username,t1.applicationlier_user_id,t1.job_id, "
    jobApplicationQuery=jobApplicationQuery+"t2.city, t1.available_from, t1.available_till, t2.education, t2.experience from "
    jobApplicationQuery=jobApplicationQuery+"job_applicationlication t1 inner join public.user t2 on t1.applicationlier_user_id=t2.id inner join job_detail t3 on "
    jobApplicationQuery=jobApplicationQuery+" t3.job_id=t1.job_id and t3.school_id='"+str(teacher.school_id)+"' and t1.job_id='"+str(job_id)+"' and t1.status='Applicationlied' order by applicationlied_on desc"
    jobApplicationlications = db.session.execute(text(jobApplicationQuery)).fetchall()

    #hired descision
    jobApplicationQueryHired = "select t1.applicationlied_on, t2.first_name, t2.last_name, t2.username,t1.applicationlier_user_id, t1.job_id, "
    jobApplicationQueryHired=jobApplicationQueryHired+"t2.city, t1.available_from, t1.available_till, t2.education, t2.experience from "
    jobApplicationQueryHired=jobApplicationQueryHired+"job_applicationlication t1 inner join public.user t2 on t1.applicationlier_user_id=t2.id inner join job_detail t3 on "
    jobApplicationQueryHired=jobApplicationQueryHired+" t3.job_id=t1.job_id and t3.school_id='"+str(teacher.school_id)+"' and t1.job_id='"+str(job_id)+"' and t1.status='Hired' order by applicationlied_on desc"
    jobApplicationlicationsHired = db.session.execute(text(jobApplicationQueryHired)).fetchall()

    #shortlist descision
    jobApplicationQueryShortlisted = "select t1.applicationlied_on, t2.first_name, t2.last_name, t2.username,t1.applicationlier_user_id, t1.job_id, "
    jobApplicationQueryShortlisted=jobApplicationQueryShortlisted+"t2.city, t1.available_from, t1.available_till, t2.education, t2.experience from "
    jobApplicationQueryShortlisted=jobApplicationQueryShortlisted+"job_applicationlication t1 inner join public.user t2 on t1.applicationlier_user_id=t2.id inner join job_detail t3 on "
    jobApplicationQueryShortlisted=jobApplicationQueryShortlisted+" t3.job_id=t1.job_id and t3.school_id='"+str(teacher.school_id)+"' and t1.job_id='"+str(job_id)+"' and t1.status='Shortlisted' order by applicationlied_on desc"
    jobApplicationlicationsShortlisted = db.session.execute(text(jobApplicationQueryShortlisted)).fetchall()

    #rejected descision
    jobApplicationQueryRejected = "select t1.applicationlied_on, t2.first_name, t2.last_name, t2.username,t1.applicationlier_user_id, t1.job_id, "
    jobApplicationQueryRejected=jobApplicationQueryRejected+"t2.city, t1.available_from, t1.available_till, t2.education, t2.experience from "
    jobApplicationQueryRejected=jobApplicationQueryRejected+"job_applicationlication t1 inner join public.user t2 on t1.applicationlier_user_id=t2.id inner join job_detail t3 on "
    jobApplicationQueryRejected=jobApplicationQueryRejected+" t3.job_id=t1.job_id and t3.school_id='"+str(teacher.school_id)+"' and t1.job_id='"+str(job_id)+"' and t1.status='Rejected' order by applicationlied_on desc"
    jobApplicationlicationsRejected = db.session.execute(text(jobApplicationQueryRejected)).fetchall()
    
    return render_template('jobApplicationlications.html', classSecCheckVal=classSecCheck(),title='Job Applicationlications',jobApplicationlications=jobApplicationlications, jobApplicationlicationsHired=jobApplicationlicationsHired,jobApplicationlicationsShortlisted= jobApplicationlicationsShortlisted, jobApplicationlicationsRejected = jobApplicationlicationsRejected )


@application.route('/jobPosts')
@login_required
def jobPosts():
    teacher=TeacherProfile.query.filter_by(user_id=current_user.id).first()
    jobPosts = JobDetail.query.filter_by(school_id=teacher.school_id).order_by(JobDetail.posted_on.desc()).all()
...
@application.route('/processApplicationlication')
@login_required
def processApplicationlication():
    applicationlier_user_id = request.args.get('applicationlier_user_id')
    job_id = request.args.get('job_id')
    process_type = request.args.get('process_type')
    #try:
    jobApplicationlicationRow = JobApplicationlication.query.filter_by(applicationlier_user_id=applicationlier_user_id, job_id=job_id).first()
    jobDetailRow = JobDetail.query.filter_by(job_id=job_id).first()
    applicationlierRow = User.query.filter_by(id=applicationlier_user_id).first()
    schoolRow = SchoolProfile.query.filter_by(school_id=jobApplicationlicationRow.school_id).first()

    print(process_type)
    if process_type=='shortlist':
        jobApplicationlicationRow.status= 'Shortlisted'
        flash('Applicationlication Shortlisted')
        try:
            applicationlication_processed(applicationlierRow.email,applicationlierRow.first_name + ' '+ applicationlierRow.last_name, schoolRow.school_name,jobDetailRow.category, 'Shortlisted')
        except:
            pass
    elif process_type=='reject':
        jobApplicationlicationRow.status= 'Rejected'
        flash('Applicationlication Rejected')
        try:
            applicationlication_processed(applicationlierRow.email,applicationlierRow.first_name + ' '+ applicationlierRow.last_name, schoolRow.school_name,jobDetailRow.category, 'Rejected')
        except:
            pass
    elif process_type =='hire':
        jobApplicationlicationRow.status= 'Hired'
        flash('Applicationlication Hired')
        try:
            applicationlication_processed(applicationlierRow.email,applicationlierRow.first_name + ' '+ applicationlierRow.last_name, schoolRow.school_name,jobDetailRow.category, 'Hired')
        except:
            pass
    else:
        flash('Error processing applicationlication idk')
    db.session.commit()
    
    return redirect(url_for('jobApplicationlications',job_id=job_id))
    #except:
    flash('Error processing applicationlication')
    return redirect(url_for('jobApplicationlications',job_id=job_id))


@application.route('/submitPost', methods=['GET', 'POST'])
@login_required
def submitPost():
    form = PostForm()
    if form.validate_on_submit():
...
@application.route('/archiveLiveClass')
@login_required
...
@application.route('/liveClass', methods=['GET','POST'])
@login_required
def liveClass():    
...
@application.route('/updateSearchIndex/<task>')
@login_required
...
@application.route('/openLiveClass')
def openLiveClass():
...
@application.route('/courseDetail')
def courseDetail():
    live_class_id = request.args.get('live_class_id')
    course_id = request.args.get('course_id')
...
@application.route('/courseTopicDetail')
...
@application.route('/fetchClassVideo',methods=['GET','POST'])
def fetchClassVideo():
    topic_id = request.args.get('topic_id')
...
@application.route('/courseBatchDetail')
...
@application.route('/studTakeQuiz', methods=['GET','POST'])
def studTakeQuiz():
...
@application.route('/addComments',methods=['GET','POST'])
def addComments():
    ...
    for comment in fetchComments:
        commentList.applicationend(str(comment.username)+":"+str(comment.comment)+":"+str(comment.last_modified_date.strftime('%d %B %Y'))+":"+str(lenComm))
    return jsonify(commentList)

@application.route('/addTopicReview',methods=['GET','POST'])
def addTopicReview():
    course_id = request.args.get('course_id')
...
@application.route('/addReviewComment',methods=['GET','POST'])
def addReviewComment():
    ...
    for review in fetchReview:
        reviewList.applicationend(str(review.username)+":"+str(review.comment)+":"+str(review.last_modified_date.strftime('%d %B %Y'))+":"+str(lenComm))
    return jsonify(reviewList)

@application.route('/addRatingReview',methods=['GET','POST'])
def addRatingReview():
    course_id = request.args.get('course_id')
...
@application.route('/batchTopicList',methods=['GET','POST'])
def batchTopicList():
    batch_id = request.args.get('batch_id')
    ...
    for topicId in topicIds:
        ...
        topics.applicationend(str(topicName.topic_name)+':'+str(topicName.topic_id))
    if topics:
        return jsonify(topics)
    else:
        return ""

@application.route('/tutorDashboard',methods=['GET','POST'])
def tutorDashboard():
...
@application.route('/fetchBatch',methods=['GET','POST'])
def fetchBatch():
...
@application.route('/createBatch',methods=['GET','POST'])
def createBatch():
...
@application.route('/teacherRegistration')
def teacherRegistration():
    ...
    if reviewStatus:
        ...
        print('if not registered as teacher')
        return render_template('teacherRegistration.html',School=School,NewSchool=NewSchool,bankDetail=bankDetail,teacherDetail=teacherDetail,teacher_id=teacher_id)
@application.route('/teacherRegForm',methods=['GET','POST'])
def teacherRegForm():
...
@application.route('/getOnlineClassLink',methods=['GET','POST'])
...
def sendSMS(message, phoneList):
    apiPath = "http://173.212.233.109/application/smsapisr/index.php?key=35EF8379A04DB8&"
    ...
    if teacherIdExist==None:
        ...
        if course_id:
            ...
            for topicId in topicsID:
                ...
                topicList.applicationend(topic_name.topic_name)
                topicList.applicationend(questionNo)
                topicList.applicationend(topicId.topic_id)
                ...
                topicList.applicationend(checkNotes)
                topicList.applicationend(checkRec)
                print(topicList)
                topicL.applicationend(topicList)
            print(topicL)
            for topic in topicL:
                print(topic[0])
                print(topic[1])
...
@application.route('/searchTopic',methods=['GET','POST'])
def searchTopic():
    ...
    for course_id in coursesId:
        if str(course_id.course_id)!=str(courseId):
            ...
            for top in TopicIds:
                print('Topic:'+str(top))
                topicArray.applicationend(str(top.topic_id)+':'+str(top.topic_name)+':'+str(course_id.course_id))
    if topicArray:
        return jsonify([topicArray])
    else:
        return ""

@application.route('/fetchQues',methods=['GET','POST'])
def fetchQues():
    teacherData = TeacherProfile.query.filter_by(user_id=current_user.id).first()
    ...
    for topic in topics:
        ...
        topicsDet.applicationend(str(topic)+':'+str(quesNo)+':'+str(topicName.topic_id)+':'+str(checkNotes)+':'+str(checkRec))
    if topicsDet:
        return jsonify(topicsDet)
    else:
        return ""

@application.route('/fetchRecording',methods=['GET','POST'])
def fetchRecording():
...
@application.route('/deleteNotes',methods=['GET','POST'])
def deleteNotes():
...
@application.route('/fetchNotes',methods=['GET','POST'])
def fetchNotes():
    ...
    for note in notes:
        ...
        notesData.applicationend(str(NewNotes)+'!'+str(note.notes_url)+'!'+str(note.tn_id))
    print(notesData)
    if notesData:
...
@application.route('/fetchRemQues',methods=['GET','POST'])
def fetchRemQues():
    ...
    for qId in quesIdList:
        ...
        quesArray.applicationend(str(quesName.question_description)+':'+str(opt1)+':'+str(opt2)+':'+str(opt3)+':'+str(opt4)+':'+str(qId))
    print('quesArray:')
    print(quesArray)
    if quesArray:
...
@application.route('/topicName',methods=['GET','POST'])
def topicName():
...
@application.route('/fetchTopicQues',methods=['GET','POST'])
def fetchTopicsQues():
    teacherData = TeacherProfile.query.filter_by(user_id=current_user.id).first()
    board = SchoolProfile.query.filter_by(school_id=teacherData.school_id).first()
    ...
    if len(quesIds)==0:
        ...
        quesArray.applicationend(str('')+':'+str(NewTopicName)+':'+str('')+':'+str('')+':'+str('')+':'+str('')+':'+str('')+':'+str(topicName.topic_id))
    for qId in quesIds:
        quesObj = {}    
        ...
        for option in quesOptions:
            ...
            print('quesOptions:'+str(option.option_desc))
        NewTopicName = topicName.topic_name.replace(",","/")
        quesArray.applicationend(str(quesName.question_description)+':'+str(NewTopicName)+':'+str(opt1)+':'+str(opt2)+':'+str(opt3)+':'+str(opt4)+':'+str(qId.question_id)+':'+str(topicName.topic_id))
    print('quesArray:')
    print(quesArray)
    if quesArray:
        return jsonify(quesArray)
...
@application.route('/deleteTopic',methods=['GET','POST'])
def deleteTopic():
...
@application.route('/updateCourseTopic',methods=['GET','POST'])
...
@application.route('/addCourseTopic',methods=['GET','POST'])
def addCourseTopic():
    teacherData = TeacherProfile.query.filter_by(user_id=current_user.id).first()
    board = SchoolProfile.query.filter_by(school_id=teacherData.school_id).first()
...
@application.route('/fetchTickCorrect',methods=['GET','POST'])
def fetchTickCorrect():
    ...
    for quesId in quesList:
        ...
        correctOpt.applicationend(str(corr.option_desc)+':'+str(quesId))
    return jsonify(correctOpt)

@application.route('/addRecording',methods=['GET','POST'])
def addRecording():
    topic_id = request.args.get('topic_id')
    course_id = request.args.get('course_id')
...
@application.route('/updateNotes',methods=['GET','POST'])
def updateNotes():
    topicId = request.args.get('topic_id')
...
@application.route('/addNotes',methods=['GET','POST'])
def addNotes():
    topicId = request.args.get('topic_id')
    notesName = request.form.getlist('notesName')
    notesURL = request.form.getlist('notesURL')
...
@application.route('/addNewQuestion',methods=['GET','POST'])
def addNewQuestion():
    teacherData = TeacherProfile.query.filter_by(user_id=current_user.id).first()
    board = SchoolProfile.query.filter_by(school_id=teacherData.school_id).first()
...
@application.route('/fetchQuesList',methods=['GET','POST'])
def fetchQuesList():
    ...
    if indic=='1':
        for quesId in quesList:
            ...
            for options in quesOptions:
                ...
                optionList.applicationend(options.option_desc)
        ...
        quesDetails.applicationend(str(quesDesc.question_description)+':'+str(op1)+':'+str(op2)+':'+str(op3)+':'+str(op4)+':'+str(corrOption))
    return jsonify([quesDetails])

@application.route('/saveAndPublishedCourse',methods=['GET','POST'])
...
@application.route('/saveCourse',methods=['GET','POST'])
def saveCourse():
    teacherData = TeacherProfile.query.filter_by(user_id=current_user.id).first()
    print('inside saveCourse')
    course = request.form.get('course')
...
@application.route('/courseEntry',methods=['GET','POST'])
def courseEntry():
...
@application.route('/myCourses')
def myCourses():

    return render_template('myCourses.html')

@application.route('/paymentForm')
def paymentForm():    
    if current_user.is_authenticated:
        ...
        applicationId= application.config['ALLLEARN_CASHFREE_APPLICATION_ID']
        returnUrl = url_for('paymentResponse',_external=True)
        notifyUrl = url_for('notifyUrl',_external=True)
        return render_template('_paymentForm.html',courseDetailData=courseDetailData,courseBatchData=courseBatchData, vendorDataEncoded=vendorDataEncoded,messageData=messageData,notifyUrl=notifyUrl,returnUrl=returnUrl, schoolData=schoolData, applicationId=applicationId, orderId = orderId, amount = amount, orderCurrency = currency, orderNote = note, customerName = payer_name)
    else:
        flash('Please login to enroll')
        return jsonify(['1'])

@application.route('/freeEnrollment')
def freeEnrollment():
    if current_user.is_authenticated:
        batch_id = request.args.get('batch_id')
        courseBatchData = CourseBatch.query.filter_by(batch_id =batch_id , is_archived='N').first()
...
def handlerequest():
    mode = application.config["MODE"] # <-------Change to TEST for test server, PROD for production
    ...
    secret = application.config['ALLLEARN_CASHFREE_SECRET_KEY'].encode('utf-8')
...
@application.route('/paymentResponse', methods=["POST"])
def paymentResponse():
...
@application.route('/notifyUrl',methods=["POST"])
def notifyUrl():
    postData = {
      "orderId" : request.form['orderId'], 
      "orderAmount" : request.form['orderAmount'], 
...
def requestSignGenerator(applicationId, orderId, orderAmount, orderCurrency, orderNote, customerName, customerPhone,customerEmail, returnUrl, notifyUrl):    
    postData = {
    ...
    #get secret key from your config
    secret = bytes(application.config['ALLLEARN_CASHFREE_SECRET_KEY'],encoding='utf-8')
...
def verifyResponseSign(receivedResponseSign, postData):
    ...
    #get secret key from your config
    secret = bytes(application.config['ALLLEARN_CASHFREE_SECRET_KEY']).encode('utf-8')
    signature = base64.b64encode(hmac.new(secret, message,digestmod=hashlib.sha256).digest())
...
@application.route('/classNotification')
def classNotification():    
    return render_template('classNotification.html')

@application.route('/sendClassNotification')
...
@application.route('/teachingApplicationlicantProfile/<user_id>')
@login_required
def teachingApplicationlicantProfile(user_id):
    job_id = request.args.get('job_id')
    user = User.query.filter_by(id=user_id).first_or_404()
    accessingUser = User.query.filter_by(id=current_user.id).first_or_404()
    return render_template('teachingApplicationlicantProfile.html',user=user, user_type_val=str(accessingUser.user_type),job_id=job_id)
...
def user(username):
    ...
    if current_user.user_type==72:
        ...
        user_type_val = current_user.user_type
    if user.user_type==161:        
        return redirect(url_for('teachingApplicationlicantProfile',title='My Profile',user_id=user.id))
    else:
...
@application.route('/setFee',methods=['GET','POST'])
def setFee():
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first() 
    class_val = request.args.get('class_val')
    section = request.args.get('section')
...
@application.route('/feeManagement')
@login_required
def feeManagement():
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first() 
...
@application.route('/sendUserNotificationEmail',methods=['POST','GET'])
...
@application.route('/sendNotificationEmail')
...
@application.route('/sendPerformanceReportEmail')
...
@application.route('/requestUserAccess')
...
@application.route('/addSyllabus',methods=['GET','POST'])
def addSyllabus():
...
@application.route('/generalSyllabusClasses')
def generalSyllabusClasses():
    ...
    for val in distinctClasses:
        classArray.applicationend(val.class_val)
    if classArray:
        return jsonify([classArray])
    else:
        return ""

@application.route('/syllabusClasses')
@login_required
def syllabusClasses():
    board_id=request.args.get('board_id')
    classSectionArray = []
    ...
    for val in distinctClasses:
        ...
        classSectionArray.applicationend(str(val.class_val)+':'+str(sectionsString))
...
@application.route('/generalSyllabusSubjects',methods=['GET','POST'])
def generalSyllabusSubjects():
    ...
    for val in subjects:
        ...
        sujectArray.applicationend(str(val.msg_id)+":"+str(val.description))
    if sujectArray:
        return jsonify([sujectArray])   
    else:
        return ""

@application.route('/syllabusSubjects')
@login_required
...
@application.route('/fetchSubjects',methods=['GET','POST'])
def fetchSubjects():
    class_val = request.args.get('class_val')
...
@application.route('/fetchRemSubjects',methods=['GET','POST'])
def fetchRemSubjects():
    ...
    for val in subjects:
        # subject = MessageDetails.query.filter_by(msg_id=val.subject_id).first()
        subjectArray.applicationend(str(val.msg_id)+":"+str(val.description))
...
@application.route('/addSubject',methods=['GET','POST'])
def addSubject():
...
@application.route('/addChapter',methods=['GET','POST'])
def addChapter():
...
@application.route('/addNewSubject',methods=['GET','POST'])
def addNewSubject():
    subject = request.args.get('subject')
...
@application.route('/addNewBook',methods=['GET','POST'])
def addNewBook():
...
@application.route('/checkForChapter',methods=['GET','POST'])
def checkForChapter():
...
@application.route('/addClassSection',methods=['POST'])
def addClassSection():
...
@application.route('/checkForBook',methods=['GET','POST'])
def checkForBook():
...
@application.route('/checkForSubject',methods=['GET','POST'])
def checkForSubject():
    subject = request.args.get('subject')
    subject = subject.title()
...
@application.route('/checkforClassSection',methods=['GET','POST'])
...
@application.route('/addNewTopic',methods=['GET','POST'])
def addNewTopic():
...
@application.route('/addNewChapter',methods=['GET','POST'])
def addNewChapter():
...
@application.route('/spellCheckBook',methods=['GET','POST'])
def spellCheckBook():
...
@application.route('/deleteSubject',methods=['GET','POST'])
def deleteSubject():
...
@application.route('/deleteBook',methods=['GET','POST'])
def deleteBook():
    subject = request.args.get('subject')
    class_val = request.args.get('class_val')
    bookId = request.args.get('bookId')
...
@application.route('/deleteTopics',methods=['GET','POST'])
def deleteTopics():
    subject = request.args.get('subject')
    class_val = request.args.get('class_val')
    bookId = request.args.get('bookId')
...
@application.route('/deleteChapters',methods=['GET','POST'])
def deleteChapters():
    subject = request.args.get('subject')
    bookId = request.args.get('bookId')
    class_val = request.args.get('class_val')
...
@application.route('/generalSyllabusBooks')
def generalSyllabusBooks():
    ...
    for val in distinctBooks:
        ...
        bookArray.applicationend(str(book_id.book_id)+':'+str(val.book_name))
    if bookArray:
        return jsonify([bookArray])  
    else:
        return ""

@application.route('/syllabusBooks')
@login_required
def syllabusBooks():
    subject_name=request.args.get('subject_name')
    class_val=request.args.get('class_val')
...
@application.route('/fetchRemBooks',methods=['GET','POST'])
def fetchRemBooks():
    class_val = request.args.get('class_val')
    subject = request.args.get('subject')
    subject_id = MessageDetails.query.filter_by(description=subject).first()
    ...
    for value in distinctBooksInBCSB:
        ...
        bookArray.applicationend(str(book_id.book_id)+':'+str(value.book_name))
    if bookArray:
        return jsonify([bookArray])
    else:
        return "" 

@application.route('/selectedChapter',methods=['GET','POST'])
def selectedChapter():
    ...
    selectedChapterArray.applicationend(str(chapter.chapter_name)+':'+str(chapter.chapter_num))
    return jsonify([selectedChapterArray])


@application.route('/fetchRemChapters',methods=['GET','POST'])
def fetchRemChapters():
    ...
    queryChapters = db.session.execute(text(queryChapters)).fetchall()
            ...
            # chapterArray.applicationend(str(chapter.chapter_num)+":"+str(chapter.chapter_name))
    ...
    for chapter in queryChapters:
        ...
        if len(queryChapters)>1:
            ...
            if len(queryBookDetails)>0:
                chapters = chapters+"/"
        chapterArray.applicationend(str(num)+":"+str(chapters))
    i=1
    for book in queryBookDetails:
        ...
        if len(queryBookDetails)>1:
            ...
            if len(queryChapters)>0:
                num = "/"+str(num)
        print(chapter)
        chapterArray.applicationend(str(num)+":"+str(chapter))
...
@application.route('/fetchBooks',methods=['GET','POST'])
def fetchBooks():
    class_val = request.args.get('class_val')
    subject = request.args.get('subject')
    subject_id = MessageDetails.query.filter_by(description=subject).first()
...
@application.route('/generalSyllabusChapters')
...
@application.route('/syllabusChapters') 
@login_required
    ...
    for book in queryBookDetails:
        ...
        if len(queryBookDetails)>1:
            if i==1:
                ...
                print(chapter)
            i=i+1
        chapterArray.applicationend(str(num)+":"+str(chapter)+';'+str(book.book_id))
...
@application.route('/chapterTopic',methods=['GET','POST'])
def chapterTopic():
    ...
    for topic in topics:
        ...
        if len(topics)>1:
            ...
            if len(remTopics)>0:
                topic_name = topic_name+"/"
        topicArray.applicationend(str(topic_id)+":"+str(topic_name))
        # topicArray.applicationend(str(topic.topic_id)+':'+str(topic.topic_name))
    j=1
    for remTopic in remTopics:
        ...
        if len(remTopics)>1:
            ...
            if len(topics)>0:
                topic_id = "/"+str(topic_id)
        topicArray.applicationend(str(topic_id)+":"+str(topic_name))
        # topicArray.applicationend(str(remTopic.topic_id)+':'+str(remTopic.topic_name))
...
@application.route('/fetchChapters',methods=['GET','POST'])
def fetchChapters():
    book_id=request.args.get('bookId')
...
@application.route('/fetchTopics',methods=['GET','POST'])
def fetchTopics():
    subject=request.args.get('subject')
    chapter_num=request.args.get('chapter_num')
...
@application.route('/generalSyllabusTopics',methods=['GET','POST'])
...
@application.route('/syllabusTopics')
@login_required
def syllabusTopics():
    subject_id=request.args.get('subject_id')
    board_id=request.args.get('board_id')
...
@application.route('/syllabusQuestionsDetails',methods=['GET','POST'])
def syllabusQuestionsDetails():
    ...
    #     questionArray.applicationend(question.question_description)
    questionDetailsArray.applicationend(str(topic_name.topic_name)+':'+str(subQuestion[0])+':'+str(objQuestion[0])+':'+str(refContent[0])+':'+str(boardName.description)+':'+str(chapter_name.chapter_name))
...
@application.route('/grantSchoolAdminAccess')
...
@application.route('/grantSchoolAccess')
...
@application.route('/grantUserAccess')
def grantUserAccess():
    username=request.args.get('username')    
    school_id=request.args.get('school_id')
    school=schoolNameVal()
...
@application.route('/questionBank',methods=['POST','GET'])
@login_required
def questionBank():
    topic_list=None
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first()
...
@application.route('/visitedQuestions',methods=['GET','POST'])
...
@application.route('/questionBankQuestions',methods=['GET','POST'])
def questionBankQuestions():
    ...
    for topic in topicList:
        ...
        questions.applicationend(questionList)
...
@application.route('/questionBankFileUpload',methods=['GET','POST'])
...
@application.route('/testBuilder',methods=['POST','GET'])
@login_required
def testBuilder():
    topic_list=None
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first()
...
@application.route('/filterQuestionsfromTopic',methods=['GET','POST'])
...
@application.route('/fetchRequiredQues',methods=['GET','POST'])
...
@application.route('/testBuilderQuestions',methods=['GET','POST'])  
...
@application.route('/testBuilderFileUpload',methods=['GET','POST'])
...
@application.route('/findSection',methods=['GET','POST'])
def findSection():
    class_val=request.args.get('class_val')
    school_id = User.query.filter_by(id=current_user.id).first()
    ...
    sec = []
    for section in sections:
        sec.applicationend(section.section)
    return jsonify(sec)

@application.route('/getQuestionDetails')
...
@application.route('/getChapterDetails')
...
@application.route('/schoolPerformanceRanking')
@login_required
...
@application.route('/attendance')
@login_required
def attendance():
    return render_template('attendance.html')

@application.route('/guardianDashboard')
@login_required
...
@application.route('/sendFeeSMS',methods=["GET","POST"])
def sendFeeSMS():
    if request.method=="POST":
        ...
        if class_sec_id !=None and class_sec_id !="":
            ...
            for phoneRow in studentPhones:
                if phoneRow.phone!=None and phoneRow.phone!='':
                    phoneList.applicationend(phoneRow.phone)
...
@application.route('/sendSMS',methods=["GET","POST"])
def sendSMS():
        # commType = request.form.get('commType')
    commType = 'sms'
...
@application.route('/sendComm',methods=["GET","POST"])
def sendComm():
    if request.method=="POST":
        commType = request.form.get('commType')
        message = request.form.get('message')
...
@application.route('/qrSessionScanner')
@login_required
def qrSessionScanner():
    return render_template('qrSessionScanner.html')


@application.route('/qrSessionScannerStudent')
@login_required
...
@application.route('/fetchStudentTestReport', methods=['GET','POST'])
def fetchStudentTestReport():
    ...
    for test in testHistory:
        ...
        testResData.applicationend(testData)
    print('Test Res Data:')
    print(testResData)
    return jsonify({'testResult':testResData})


@application.route('/viewHomework')
@login_required
def viewHomework():
    return render_template('viewHomework.html')

@application.route('/mobFeedbackCollection', methods=['GET', 'POST'])
...
@application.route('/startPracticeTest', methods=['GET', 'POST'])
def startPracticeTest():
    ...
    if current_user.is_anonymous:
        ...
        studentData = StudentProfile.query.filter_by(user_id=application.config['ANONYMOUS_USERID']).first()
    else:
        ...
        if questionList:  
            for q in questionList:
                if len(questions)< int(qcount):
                    questions.applicationend(q)  
    for val in questions:        
        print(str(val))
        total_marks = total_marks + val.suggested_weightage
        questionIDList.applicationend(val.question_id)
    ##Create test
...
@application.route('/feedbackCollectionStudDev', methods=['GET', 'POST'])
...
@application.route('/updateQuestion')
def updateQuestion():
    ...
    if corrans:
        ...
        if opId1 and opId2 and opId3 and opId4:
            ...
            optionlist = []
            optionlist.applicationend(op1)
            optionlist.applicationend(op2)
            optionlist.applicationend(op3)
            optionlist.applicationend(op4)
            corrAns = 'Y'
...
@application.route('/questionOptions')
def questionOptions():
    ...
    for value in questionOptionResults:
        ...
        questionOptionsList.applicationend(value.option+". "+value.option_desc)
...
@application.route('/deleteQuestion')
def deleteQuestion():
    question_id = request.args.get('question_id')
...
@application.route('/questionDetails')
def questionDetails():
    flag = True
    question_id = request.args.get('question_id')
    print("Question Id-:"+question_id)
...
@application.route('/updateStudentProfile')
...
# @application.route('/topperListAll')
# def topperListAll():
#     user = User.query.filter_by(username=current_user.username).first_or_404()
#     teacher= TeacherProfile.query.filter_by(user_id=user.id).first() 
                ...
                for col in subColumn:
                    if i!=len(subColumn)/2:
                        ...
                        if(c[0]==str(sub.msg_id)):
                            #print(c[0])
                            #print(sub.msg_id)                                                        
                            li.applicationend(sub.msg_id)
                            li.applicationend(sub.description)
                            subj.applicationend(li)
                            break
                    i=i+1
...
@application.route('/classDelivery',methods=['GET','POST'])
@login_required
def classDelivery():
    form = ContentManager()
...
@application.route('/contentManager',methods=['GET','POST'])
@login_required
def contentManager():
    topic_list=None
    user_type_val = current_user.user_type
...
@application.route('/loadContent',methods=['GET','POST'])
def loadContent():
...
@application.route('/getContentDetails',methods=['GET','POST'])
...
@application.route('/contentDetails',methods=['GET','POST'])
def contentDetails():
...
@application.route('/filterContentfromTopic',methods=['GET','POST'])
def filterContentfromTopic():
    ...
    for topic in topic_list:
        ...
        contentDetail = db.session.execute(text(content)).fetchall()
        if contentDetail:
            contents.applicationend(contentDetail)
...
@application.route('/recentContentDetails',methods=['GET','POST'])
...
@application.route('/deleteContentById',methods=['GET','POST'])
...
@application.route('/contentManagerDetails',methods=['GET','POST'])
def contentManagerDetails():
    ...
    for topic in topicList:
        ...
        if len(contentList)!=0:
            contents.applicationend(contentList)
    if len(contents)==0:
        print("No data present in the content manager details")
        return jsonify(["NA"])
    else:
...
@application.route('/existedTestPaperLinkGenerate',methods=['POST'])
...
# Start API
@application.route('/testApplication',methods=['POST'])
def testApplication():
    if request.method == 'POST':
        jsonExamData = request.json
...
@application.route('/getLeaderBoardLink',methods=['GET','POST'])
@login_required
...
@application.route('/getCustomerSupportLink',methods=['GET','POST'])
@login_required
...
@application.route('/getTopicList',methods=['POST','GET'])
def getTopicList():
    if request.method == 'POST':
        ...
        for classlist in classesListData:
            ...
            classList.applicationend(classVal)
            j=j+1
        for clas in dataList['results'].values():
            selectedOptions.applicationend(clas)
        selClass = ''
        selSubject = ''
        ...
        for subj in subjectData:
            ...
            subjectList.applicationend(sub)
            k=k+1
        ...
        for chapterDet in extractChapterData:
            ...
            chapterDetList.applicationend(chap)
            c=c+1
...
@application.route('/getSubjectsList',methods=['POST','GET'])
def getSubjectsList():
    if request.method == 'POST':
        ...
        for classlist in classesListData:
            ...
            j=j+1
        for clas in dataList['results'].values():
            selectedClassOption.applicationend(clas)
        selClass = ''
...
@application.route('/getStudentDetails',methods=['POST','GET'])
def getStudentDetails():
    if request.method == 'POST':
        ...
        for data in dataList['results'].values():
            selectedStudentOption.applicationend(data)
        conList = []
        ...
        for student in studentListQuery:
            ...
            studentList.applicationend(stud)
            l=l+1
        selStudentId = ''
...
@application.route('/getStudentsList',methods=['GET','POST'])
def getStudentsList():
    if request.method == 'POST':
...
@application.route('/getClassSectionList',methods=['POST','GET'])
...
@application.route('/getClassList',methods=['POST','GET'])
def getClassList():
    if request.method == 'POST':
...
# @application.route('/getReqTopicList',methods=['POST','GET'])
# def getReqTopicList():
#     if request.method == 'POST':
#         jsonData = request.json
#         a = json.dumps(jsonData)
...
@application.route('/getStudentPerformance',methods=['POST','GET'])
...
# @application.route('/getStudentDashboard',methods=['POST','GET'])
...
@application.route('/getStudentDet',methods=['POST','GET'])
def getStudentDet():
    if request.method == 'POST':
        print('inside getStudentDetails')
        jsonExamData = request.json
...
@application.route('/getUserDetails',methods=['POST','GET'])
def getUserDetails():
    if request.method == 'POST':
...
@application.route('/getEnteredTopicList',methods=['POST','GET'])
def getEnteredTopicList():
    if request.method == 'POST':
        ...
        for linkData in newData['url'].values():
            linkList.applicationend(linkData)
...
@application.route('/newTestLinkGenerate',methods=['POST','GET'])
...
@application.route('/getTestPaperLink',methods=['POST','GET'])
...
@application.route('/testLinkWhatsapplicationBot', methods=['POST','GET'])
def testLinkWhatsapplicationBot(): 
    phone = request.args.get('phone') 
    ...
    for fetchIds in fetchQuesIds:
        quesIds.applicationend(fetchIds.question_id)
    ...
    resultStatus = request.args.get('resultStatus')
    advance = request.args.get('advance')
    print('inside testLinkWhatsapplicationBot')
...
@application.route('/feedbackCollection', methods=['GET', 'POST'])
@login_required
def feedbackCollection():    
    if request.method=='POST':
        ...
        for questValue in questionIDList:
            ...
            questionList.applicationend(questValue.question_id)
...
@application.route('/markSessionComplete')
@login_required
...
@application.route('/checkQuestionChange')
@login_required
...
@application.route('/loadQuestionExtCam')
@login_required
...
@application.route('/loadQuestion')
@login_required
def loadQuestion():
    question_id = request.args.get('question_id')
    totalQCount = request.args.get('total')
...
@application.route('/loadQuestionStud')
def loadQuestionStud():
    ...
    if current_user.is_anonymous and student_id=='':     
        ...
        studentRow=StudentProfile.query.filter_by(user_id=application.config['ANONYMOUS_USERID']).first()
    else:
    ...
    if int(qnum)<= int(totalQCount):
        ...
        for row in answerRes:
            answer_pair = []
            answer_pair.applicationend(row.question_id)
            answer_pair.applicationend(row.answer_status)
            answer_list.applicationend(answer_pair)
...
#@application.route('/responseStudUpdate')
#@login_required
...
@application.route('/questionAllDetails')
...
#@application.route('/decodes', methods=['GET', 'POST'])
#def decodeAjax():
#    if request.method == 'POST':
...
@application.route('/responseDBUpdate', methods=['POST'])
...
@application.route('/feedbackReport')
def feedbackReport():    
...
@application.route('/reviewSubjective',methods=['GET','POST'])
...
@application.route('/studentDashboard',methods=['GET','POST'])
@login_required
...
@application.route('/addSubjMarks',methods=['GET','POST'])
def addSubjMarks():
...
@application.route('/studentFeedbackReport')
@login_required
...
@application.route('/testPerformance')
@login_required
def testPerformance():
    user = User.query.filter_by(username=current_user.username).first_or_404()        
    teacher= TeacherProfile.query.filter_by(user_id=user.id).first()    
...
@application.route('/testPerformanceGraph')
@login_required
def testPerformanceGraph():  
    ...
    if len(testPerformanceRecords) !=0:
        ...
        for subVal in distinct_subjects:
            ...
            subLevelData.applicationend(tempDict)
...
@application.route('/studentPerformanceGraph')
@login_required
...
@application.route('/classPerformance')
@login_required
...
@application.route('/resultUpload',methods=['POST','GET'])
@login_required
def resultUpload():
...
@application.route('/resultUpload/<class_val>')
def section(class_val):
    if class_val!='All':
        ...
        for section in sections:
            ...
            sectionArray.applicationend(sectionObj)
...
@application.route('/fetchStudentsName')
def fetchStudentsName():
    ...
    for student in studentList:
        ...
        studentArray.applicationend(studentObject)
    return jsonify({'studentData':studentArray})




@application.route('/fetchTestDates')
def fetchTestDates():
    ...
    for data in dateData:
        ...
        dateArray.applicationend(dateObject)
    return jsonify({'dateData':dateArray})


@application.route('/scoreGraph')
def scoreGraph():
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first()
    class_value = request.args.get('class_val')
    section = request.args.get('section')
...
@application.route('/linkWithImpact/<int:impactSchoolID>', methods=["GET","POST"])
@login_required
def linkWithImpact(impactSchoolID):
    ...
    if teacherProfileData!=None:
        ...
        status = 1
    urlForImpact = application.config['IMPACT_HOST']
...
@application.route('/api/schoolRegistration/<int:impactSchoolID>',methods=["GET","POST"])
...
@application.route('/api/userRegistration',methods=["POST"])
def apiUserRegistration():
    if request.form['internalKey'] == application.config['ALLLEARN_INTERNAL_KEY']:
...
@application.route('/api/performanceSummary/<int:schoolID>')
...
@application.route('/testDateSearch/<class_val>')
def testDate(class_val):
    ...
    for test in testdates:
        ...
        dates.applicationend(test)
    testdateArray = []
    print(dates)
    for testdate in dates:
        testdateObj = {}
        testdateObj['date'] = testdate
        testdateArray.applicationend(testdateObj)
    return jsonify({'testdates' : testdateArray})

@application.route('/resultUploadHistory')
...
@application.route('/uploadHistoryDetail',methods=['POST','GET'])
...
@application.route('/studentList/<class_val>/<section>/')
def studentList(class_val,section):
    ...
    for student in students:
        ...
        studentArray.applicationend(studentObj)

    print(str(studentArray))
    return jsonify({'students' : studentArray})

@application.route('/quesFileUpload',methods=['POST','GET'])
def quesFileUpload():
...
@application.route('/createQuestion',methods=['POST','GET'])
def createQuestion():
    weightage = request.args.get('weightage')
...
@application.route('/questionBuilder',methods=['POST','GET'])
@login_required
def questionBuilder():
    form=QuestionBuilderQueryForm()
...
@application.route('/questionUpload',methods=['GET'])
def questionUpload():
    flag = False
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first()
    form=QuestionBuilderQueryForm()
...
@application.route('/uploadMarks',methods=['GET'])
def uploadMarks():
    user = User.query.filter_by(username=current_user.username).first_or_404()    
    teacher= TeacherProfile.query.filter_by(user_id=user.id).first()     
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first()
    ...
    for student_id in student_list:
        list_id.applicationend(student_id.student_id)
    paper = ''
...
@application.route('/questionFile',methods=['GET']) 
def questionFile():
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first()
    form=QuestionBuilderQueryForm()
    form.class_val.choices = [(str(i.class_val), "Class "+str(i.class_val)) for i in ClassSection.query.with_entities(ClassSection.class_val).distinct().order_by(ClassSection.class_val).filter_by(school_id=teacher_id.school_id).all()]
...
@application.route('/addChapterTopics',methods=['GET','POST'])
def addChapterTopics():
    ...
    for chapter in chapters:
        if len(chapters)>1:
            ...
            book = chapter.book_name
            ch = chapter.topic_id
        chaptersArray.applicationend(str(book)+"@"+str(chapter.topic_name)+"@"+str(chapter.chapter_name)+"@"+str(ch))
...
@application.route('/addClass',methods=['GET','POST'])
def addClass():
    class_val = request.args.get('class_val')
    ...
    for subject in subjects:
        ...
        subjectArray.applicationend(str(subject.subject_id)+":"+str(subject_name.subject_name))
...
@application.route('/topperListForAll')
...
@application.route('/questionBuilder/<class_val>')
    ...
    if class_val!='All':
        ...
        for id in subject_id:
            ...
            if subject_name in subject_name_list:
                continue
            subject_name_list.applicationend(subject_name)
        subjectArray = []
        print('Subject Array:')
        print(subjectArray)
        for subject in subject_name_list:
            ...
            subjectArray.applicationend(subjectObj)
...
@application.route('/questionBuilder/<class_val>/<subject_id>')
def topic_list(class_val,subject_id):
    ...
    for topic in topic_list:
        ...
        topicArray.applicationend(topicObj)
    
    return jsonify({'topics':topicArray})

@application.route('/questionTopicPicker')
...
# @application.route('/coveredTopic',methods=['GET','POST'])
# def coveredTopic():
...
@application.route('/questionChapterpicker/<class_val>/<subject_id>')
def chapter_list(class_val,subject_id):
    ...
    for chapterno in chapter_num_list:
        ...
        chapter_num_array.applicationend(chapterNo)
    return jsonify({'chapterNum':chapter_num_array})

@application.route('/addEvent', methods = ["GET","POST"])
@login_required
def addEvent():        
    teacher_id=TeacherProfile.query.filter_by(user_id=current_user.id).first()
    form = addEventForm()
...
@application.route('/studentProfileOld')
@login_required
def studentProfileOld():
    return render_template('studentProfile.html')


@application.route('/allocateStudentToSponsor')
...
@application.route('/indivStudentProfile')
@login_required
def indivStudentProfile():    
    ...
    #Sponsor allocation
    urlForAllocationComplete = str(application.config['IMPACT_HOST']) + '/responseStudentAllocate'
    overallSum = 0
    overallPerfValue = 0
    sumMarks = 0
    sum1 = 0
    ...
    for n in x:               
        if studentProfileRow!=None and studentProfileRow!="":
            ...
            optionURL=""
        qrArray.applicationend(optionURL) 
        #print(optionURL)
...
@application.route('/attendanceReport',methods=["GET","POST"])
...
@application.route('/addAttendence',methods=["GET","POST"])
def addAttendence():
    class_sec_id = request.args.get('class_sec_id')
    teacherDataRow=TeacherProfile.query.filter_by(user_id=current_user.id).first() 
...
@application.route('/fetchAttendenceList',methods=["GET","POST"])
...
@application.route('/addStudentRemarks',methods = ["GET","POST"])
...
def classChecker(available_class):
    ...
    for k in available_class:
        if k.class_val == '99':
            class_list.applicationend((str('LKG')+"-"+str(k.section),str('LKG')+"-"+str(k.section)))
            print('inside available classes')
        elif k.class_val == '100':
            class_list.applicationend((str('UKG')+"-"+str(k.section),str('UKG')+"-"+str(k.section)))
        else:
            class_list.applicationend((str(k.class_val)+"-"+str(k.section),str(k.class_val)+"-"+str(k.section)))
    return class_list

@application.route('/studentProfile') 
@login_required
def studentProfile():    
...
@application.route('/studentSurveys')
def studentSurveys():
    teacherRow=TeacherProfile.query.filter_by(user_id=current_user.id).first()
...
@application.route('/indivSurveyDetail/')
...
@application.route('/updateSurveyAnswer',methods=["GET","POST"])
...
@application.route('/addNewSurvey',methods=["GET","POST"])
def addNewSurvey():    
    teacherRow=TeacherProfile.query.filter_by(user_id=current_user.id).first()
...
@application.route('/archiveSurvey')
def archiveSurvey():
    survey_id = request.args.get('survey_id')
...
@application.route('/inventoryManagement')
...
@application.route('/addInventoryItem', methods=["GET","POST"])
...
@application.route('/archiveInventory')
...
@application.route('/studentInventoryAlloc')
...
@application.route('/updateInventoryAllocation')
...
@application.route('/paymentSuccess')
@login_required
def paymentSuccess():
    return render_template('paymentSuccess.html')

@application.route('/paymentFailure')
@login_required
def paymentFailure():
    return render_template('paymentFailure.html')


@application.route('/createSubscription',methods = ["GET","POST"])
@login_required
...
@application.route('/studentHomeWork')
@login_required
def studentHomeWork():
...
@application.route('/homeworkReview')
@login_required
def homeworkReview():
...
@application.route('/indivHomeworkReview',methods=['GET','POST'])
@login_required
...
@application.route('/indivHomeworkDetail',methods=['GET','POST'])
@login_required
...
@application.route('/addAnswerRemark',methods=["GET","POST"])
def addAnswerRemark():
...
checkValue = ''
@application.route('/addHomeworkAnswer',methods=["GET","POST"])
...
@application.route('/addNewHomeWork',methods=["GET","POST"])
def addNewHomeWork():     
    teacherRow=TeacherProfile.query.filter_by(user_id=current_user.id).first()
    questions = request.form.getlist('questionInput')
...
@application.route('/archiveHomeWork')
def archiveHomeWork():
    homework_id = request.args.get('homework_id')
...
def conversion(num):
    strings = []
    for integ in num:
        strings.applicationend(str(integ))
...
def updateSchedule():
    ...
    while p<len(slotsoutput):
        ...
        if r not in indexSlot:
            p=p+1
            indexSlot.applicationend(r)
        # print(indexSlot)
    for index in indexSlot:
        finalSlot.applicationend(slotsoutput[index])
        # print(finalSlot)
...
@application.route('/fetchTimeTable',methods=['GET','POST'])
def fetchTimeTable():
...
@application.route('/downloadTimeTable',methods=['GET','POST'])
...
@application.route('/allSubjects',methods=['GET','POST'])
def allSubjects():
    ...
    for subject in subjects:
        ...
        subjectList.applicationend(str(subject_name.description))
    return jsonify([subjectList])

#End
#Start

@application.route('/addTeacherClassSubject',methods=['GET','POST'])
...
@application.route('/loadSubject',methods=['GET','POST'])
def loadSubject():
    teacher = TeacherProfile.query.filter_by(user_id=current_user.id).first()
    teacher_id = request.args.get('teacher_id')
    class_sec_id = request.args.get('class_sec_id')
    ...
    for subjects in allSubjects:
        ...
        subject_name = db.session.execute(text(subject_name)).first()
        selArray.applicationend(str(subject_name.description)+':'+str('false'))
    for sub in selSubjects:
        subject_name = "select description from message_detail where msg_id='"+str(sub.subject_id)+"'"
        subject_name = db.session.execute(text(subject_name)).first()
        selArray.applicationend(str(subject_name.description)+':'+str('true'))
    if selArray:
        return jsonify([selArray])
    else:
        return ""

@application.route('/loadClasses',methods=['GET','POST'])
def loadClasses():
    teacher_id = request.args.get('teacher_id')
    teacher = TeacherProfile.query.filter_by(user_id=current_user.id).first()
    ...
    for clas in class_val: 
        ...
        if teacher:
            classes.applicationend(str(clas.class_val)+':'+str(clas.class_sec_id)+':'+str(clas.section)+':'+str('true'))
        else:
            classes.applicationend(str(clas.class_val)+':'+str(clas.class_sec_id)+':'+str(clas.section)+':'+str('false'))
        
    return jsonify([classes])

@application.route('/teacherAllocation',methods=['GET','POST'])
...
@application.route('/subscriptionPlans')
...
@application.route('/studTC',methods=["GET","POST"])
@login_required
def studTC():
    teacherData = TeacherProfile.query.filter_by(user_id=current_user.id).first()
...
@application.route('/archiveTCClass',methods=["GET","POST"])
@login_required
def archiveTCClass():
...
@application.route('/accessStudTC')
def accessStudTC():
    school_id = request.args.get('school_id')
...
if __name__=="__main__":
    ...
    #application.run(host=os.getenv('IP', '127.0.0.1'), 
